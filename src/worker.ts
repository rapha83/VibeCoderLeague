...
const MANUAL_SYNC_PAGES_PER_CONSENT = 2;
...
export function createApp(options: AppOptions = {}) {
  ...
  app.post("/api/selections", async c => {
    ...
    await c.env.DB.batch([c.env.DB.prepare("INSERT INTO participants(github_id,github_login,avatar_url,display_name,public_profile_id,consent_active,withdrawn_at,updated_at) VALUES(?,?,?,?,?,1,NULL,?) ON CONFLICT(github_id) DO UPDATE SET github_login=excluded.github_login,avatar_url=excluded.avatar_url,display_name=excluded.display_name,public_profile_id=COALESCE(participants.public_profile_id,excluded.public_profile_id),consent_active=1,withdrawn_at=NULL,updated_at=excluded.updated_at").bind(s.github_id,s.github_login,s.avatar_url,s.github_login,random(),timestamp), c.env.DB.prepare("UPDATE consents SET active=0,updated_at=? WHERE github_id=? AND repo_id<>? AND active=1").bind(timestamp,s.github_id,repo.id), c.env.DB.prepare("DELETE FROM pull_requests WHERE author_id=? AND repo_id<>?").bind(s.github_id,repo.id), c.env.DB.prepare("INSERT INTO consents(github_id,repo_id,repo_name,installation_id,visibility,declared_tooling,declared_model,active,updated_at) VALUES(?,?,?,?, 'public',?,?,1,?) ON CONFLICT(github_id,repo_id) DO UPDATE SET installation_id=excluded.installation_id,active=1,visibility='public',declared_tooling=excluded.declared_tooling,declared_model=excluded.declared_model,updated_at=excluded.updated_at").bind(s.github_id,repo.id,repo.nameWithOwner,candidate.installationId,tooling,model,timestamp)]);
...
export type SyncOutcome = { status: "busy" | "partial" | "complete" | "retracted" };
export async function syncConsent(env: Env, consent: { github_id: string; repo_id: string; repo_name: string; installation_id: string; declared_tooling: string | null; declared_model: string | null }, token: string, maxPages = SYNC_PAGES_PER_CONSENT, budget?: SyncWorkBudget): Promise<SyncOutcome> {
  ...
  const active = await env.DB.prepare("SELECT 1 FROM participants p JOIN consents c ON c.github_id=p.github_id WHERE p.github_id=? AND p.consent_active=1 AND c.repo_id=? AND c.active=1 AND c.visibility='public'").bind(consent.github_id,consent.repo_id).first();
  ...
  for (let page = 0; page < maxPages; page++) { if (budget && !budget.tryConsume()) { await releasePartialLease(); return { status: "partial" }; } let result; await renewLease(); try { result = await client.pulls(repo, cursor); } catch (error) { if (error instanceof Error && error.message === "repo_inaccessible") return retract(); await releasePartialLease(); throw error; } await renewLease(); if (result.visibility !== "PUBLIC") return retract();
    ...
    if (!progress.meta.changes) throw new Error("sync_lease_lost"); if (terminal) return { status: complete ? "complete" : "partial" };
  }
