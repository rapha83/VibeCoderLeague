...
export async function syncConsent(env: Env, consent: { github_id: string; repo_id: string; repo_name: string; installation_id: string; declared_tooling: string | null; declared_model: string | null }, token: string, maxPages = 2) {
  ...
  for (let page = 0; page < maxPages; page++) { let result; await renewLease(); try { result = await client.pulls(repo, cursor); } catch (error) { if (error instanceof Error && error.message === "repo_inaccessible") { await retract(); return; } throw error; } await renewLease(); if (result.visibility !== "PUBLIC") { await retract(); return; }
